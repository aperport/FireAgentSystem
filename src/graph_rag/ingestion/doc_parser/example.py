"""
数据准备模块 — 通用文档加载、切分与元数据增强工具。

⚠️ 注意：本模块属于食谱/烹饪领域（分类：荤菜/素菜/汤品，难度：星级评定），
与当前消防后勤知识问答系统无关。可能是从其他项目迁移过来的参考代码。

建议处理：
    1. 移除本文件，消防场景不需要食谱分类逻辑
    2. 或保留作为参考实现（MarkdownHeaderTextSplitter 用法、父子文档映射等），
       但应重命名为 _example.py 或移至 tests/ 目录

可借鉴的实现模式（如保留参考）：
    - load_documents()：递归加载 Markdown 文件 + 基于 MD5 的稳定 parent_id
    - _enhance_metadata()：从路径/内容提取分类元数据
    - _markdown_header_split()：使用 MarkdownHeaderTextSplitter 按标题切分
    - get_parent_documents()：根据子块回查父文档（与 context_fusion 的父文档回填类似）
    - export_metadata()：元数据导出为 JSON
